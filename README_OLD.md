...
**Live Sites:**
- Bauturbo: https://leitfaden.umsetzungslabor-bauturbo.de (pt-web-1)
...
└── Separate Build Outputs per Site (_site/)
    ├── bauturbo/ → leitfaden.umsetzungslabor-bauturbo.de (pt-web-1)
...
- Built sites in `_site/` subdirectories:
  - `_site/bauturbo/` → leitfaden.umsetzungslabor-bauturbo.de
...
- ✅ SSL certificates configured via certbot:
  - `leitfaden.umsetzungslabor-bauturbo.de`
...
**Configuration** (`/etc/nginx/sites-available/leitfaden.umsetzungslabor-bauturbo.de`):
```nginx
server {
    listen 80;
    listen [::]:80;
    server_name leitfaden.umsetzungslabor-bauturbo.de;
...
server {
    ...
    ssl_certificate /etc/letsencrypt/live/leitfaden.umsetzungslabor-bauturbo.de/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/leitfaden.umsetzungslabor-bauturbo.de/privkey.pem;
...
```bash
# Bauturbo
sudo certbot --nginx -d leitfaden.umsetzungslabor-bauturbo.de
